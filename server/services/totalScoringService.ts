...
export class TotalScoringService {
  ...
  static calculateTotalScore(
    ...
    const totalScore = 
      (individualScores.openToWorkScore * normalizedWeights.openToWork) +
   ...
   */
  static calculatePriority(totalScore: number): string {
    if (totalScore >= 7.5) return 'High';
    if (totalScore >= 5.0) return 'Medium';
    return 'Low';
  }
   ...
   */
  static async updateCandidateSkillMatchAndTotal(
    candidateId: string,
    comId: string,
    skillMatchScore: number,
   ...
   */
  static async recalculateAllCandidateScores(comId: string): Promise<{
    success: boolean;
    updatedCount: number;
    weights: ScoringWeights;
  }> {
