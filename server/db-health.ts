...
export async function checkDatabaseHealth(): Promise<DatabaseHealth> {
  try {
    ...
    const tables = [
      { 
        name: 'candidates', 
        count: parseInt(candidatesResult.rows[0].count as string),
      },
      { 
        name: 'projects', 
        count: parseInt(projectsResult.rows[0].count as string),
      },
      { 
        name: 'processing_jobs', 
        count: parseInt(jobsResult.rows[0].count as string),
      },
      { 
        name: 'activities', 
        count: parseInt(activitiesResult.rows[0].count as string),
      },
      { 
        name: 'users', 
        count: parseInt(usersResult.rows[0].count as string),
      }
    ];
...
export async function optimizeDatabase(): Promise<{ success: boolean; operations: string[] }> {
