...
export class EeezoService {
     ...
     */
    static async processEeezoResume(request) {
        try {
     ...
     */
    static async enrichCandidateInBackground(candidateId, extractedData) {
        try {
            ...
            const linkedInProfile = await linkedInService.enrichProfile(extractedData.linkedinUrl, extractedData.name, extractedData.experience?.[0]?.company, extractedData.title, extractedData.location, [{ id: candidateId, ...extractedData }]);
            if (linkedInProfile) {
                ...
                if (linkedInProfile.name)
                    updateData.name = linkedInProfile.name;
                ...
                if (linkedInProfile.location)
                    updateData.location = linkedInProfile.location;
                ...
                if (linkedInProfile.profileUrl)
                    updateData.linkedinUrl = linkedInProfile.profileUrl;
                if (linkedInProfile.openToWork !== undefined)
                    updateData.openToWork = linkedInProfile.openToWork;
                if (linkedInProfile.lastActive)
                    updateData.lastActive = linkedInProfile.lastActive;
                if (linkedInProfile.connections)
                    updateData.linkedinConnections = linkedInProfile.connections;
                // Add LinkedIn-specific fields
                if (linkedInProfile.headline)
                    updateData.linkedinHeadline = linkedInProfile.headline;
                if (linkedInProfile.about)
                    updateData.linkedinSummary = linkedInProfile.about;
                if (linkedInProfile.lastActive) {
                    try {
     ...
     */
    static async getCandidatesByCompanyId(comId, options = {}) {
        try {
            let query = db.select().from(candidates)
                .where(eq(candidates.comId, comId))
                .orderBy(candidates.createdAt);
            if (options.status) {
                query = query.where(and(eq(candidates.comId, comId), eq(candidates.eezoStatus, options.status)));
            }
            if (options.limit) {
     ...
     */
    static async getEeezoProcessingStatus(comId) {
        try {
     ...
     */
    static async updateCandidateEeezoStatus(candidateId, updateData) {
        try {
