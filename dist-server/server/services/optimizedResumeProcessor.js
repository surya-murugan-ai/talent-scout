...
export class OptimizedResumeProcessor {
     ...
     */
    async processSingleResume(buffer, filename, options = {}) {
        ...
        try {
            ...
            const processedCandidates = await this.processCandidatesOptimized(candidates, weights, options);
     ...
     */
    async processCandidatesOptimized(candidates, weights, options) {
        const batchSize = options.batchSize || 5;
        const processedCandidates = [];
        // Process in batches for better performance
        for (let i = 0; i < candidates.length; i += batchSize) {
            const batch = candidates.slice(i, i + batchSize);
            options.progressCallback?.(25 + Math.round((i / candidates.length) * 70), `Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(candidates.length / batchSize)}`);
            if (options.parallelProcessing) {
                // Process batch in parallel
                const batchPromises = batch.map(candidate => this.processSingleCandidate(candidate, weights, options));
     ...
     */
    async processSingleCandidate(candidateData, weights, options) {
        try {
     ...
     */
    calculateHireabilityScore(resumeData, linkedInProfile, analysis) {
        let score = 0;
     ...
     */
    calculateHireabilityFactors(resumeData, linkedInProfile, analysis) {
        return {
