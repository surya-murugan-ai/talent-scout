...
export class CompanyAnalysisService {
   ...
   */
  static analyzeCompanyDifference(
    resumeCompany: string | null | undefined,
    linkedinCompany: string | null | undefined
  ): { difference: string; score: number } {
   ...
   */
  static assessHireability(
    openToWork: boolean,
    companyDifferenceScore: number,
    linkedinLastActive: Date | null,
    ...
    let potentialToJoin: 'High' | 'Medium' | 'Low' | 'Unknown' = 'Unknown';
   ...
   */
  private static isPartialMatch(company1: string, company2: string): boolean {
   ...
   */
  private static isRecentCompanyChange(company1: string, company2: string): boolean {
   ...
   */
  private static getCompanyDifferenceDescription(score: number): string {
    if (score >= 9) return "Companies match perfectly";
    if (score >= 7) return "Companies likely match";
    if (score >= 5) return "Companies partially match";
    if (score >= 3) return "Companies are different";
   ...
   */
  static async updateCandidateAnalysis(candidateId: string): Promise<void> {
    try {
      ...
      const companyAnalysis = this.analyzeCompanyDifference(
        candidate.company,
        candidate.currentEmployer
      );
      
      // Assess hireability
      const hireabilityAnalysis = this.assessHireability(
