...
export class JobStabilityService {
     ...
     */
    static calculateJobStabilityScore(experience) {
        ...
        const finalScore = 0.30 * avgTenureScore +
     ...
     */
    static calculateMetrics(experience) {
        ...
        const recentJobs = jobs.filter(job => job.startDate && new Date(job.startDate) >= fiveYearsAgo);
     ...
     */
    static parseExperienceData(experience) {
     ...
     */
    static calculateTenureMonths(startDate, endDate) {
        if (!startDate)
            return 0;
     ...
     */
    static calculateLargestGap(jobs) {
        if (jobs.length < 2)
            return 0;
     ...
     */
    static calculateContinuityScore(jobs) {
        if (jobs.length <= 1)
            return 100;
        ...
        if (bestCategoryScore >= 70)
            return 100;
        if (bestCategoryScore >= 50)
            return 70;
        if (bestCategoryScore >= 30)
            return 40;
        return 20;
    }
    /**
     ...
     */
    static scoreAverageTenure(avgTenure) {
        if (avgTenure >= 24)
            return 100;
        if (avgTenure >= 18)
            return 75;
     ...
     */
    static scoreLongestTenure(longestTenure) {
        if (longestTenure >= 60)
            return 100; // 5+ years
        if (longestTenure >= 36)
            return 80; // 3-5 years
     ...
     */
    static scoreJobChanges(changes) {
        if (changes <= 1)
            return 100;
        if (changes === 2)
            return 80;
     ...
     */
    static scoreGaps(largestGap) {
        if (largestGap === 0)
            return 100; // No gaps
        if (largestGap <= 6)
            return 70; // ≤6 months
        if (largestGap <= 12)
            return 40; // 7-12 months
        return 20; // >12 months
    }
}
